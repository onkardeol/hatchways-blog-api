# ENDPOINT: /api/posts?tags=design&sortBy=likes&direction=desc
API_RESPONSE_ALL_PARAMS = {
    "posts": [
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 98,
            "likes": 934,
            "popularity": 0.5,
            "reads": 17307,
            "tags": ["design"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 11,
            "likes": 750,
            "popularity": 0.54,
            "reads": 6183,
            "tags": ["science", "design"],
        },
        {
            "author": "Adalyn Blevins",
            "authorId": 11,
            "id": 16,
            "likes": 749,
            "popularity": 0.29,
            "reads": 71754,
            "tags": ["design", "history"],
        },
        {
            "author": "Kinley Crosby",
            "authorId": 10,
            "id": 75,
            "likes": 733,
            "popularity": 0.98,
            "reads": 94910,
            "tags": ["science", "design", "culture"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 4,
            "likes": 728,
            "popularity": 0.88,
            "reads": 19645,
            "tags": ["science", "design", "tech"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 41,
            "likes": 715,
            "popularity": 0.69,
            "reads": 47876,
            "tags": ["design", "health"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 28,
            "likes": 713,
            "popularity": 0.8,
            "reads": 89173,
            "tags": ["design"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 36,
            "likes": 709,
            "popularity": 0.08,
            "reads": 65277,
            "tags": ["health", "design"],
        },
        {
            "author": "Zackery Turner",
            "authorId": 12,
            "id": 63,
            "likes": 682,
            "popularity": 0.62,
            "reads": 54374,
            "tags": ["culture", "design"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 100,
            "likes": 573,
            "popularity": 0.43,
            "reads": 89894,
            "tags": ["science", "design", "history"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 81,
            "likes": 552,
            "popularity": 0.09,
            "reads": 22975,
            "tags": ["design", "history"],
        },
        {
            "author": "Jaden Bryant",
            "authorId": 3,
            "id": 51,
            "likes": 487,
            "popularity": 0.01,
            "reads": 98798,
            "tags": ["design", "startups", "tech"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 42,
            "likes": 452,
            "popularity": 0.08,
            "reads": 39721,
            "tags": ["design"],
        },
        {
            "author": "Lainey Ritter",
            "authorId": 1,
            "id": 97,
            "likes": 382,
            "popularity": 0.83,
            "reads": 47484,
            "tags": ["politics", "science", "design", "culture"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 56,
            "likes": 319,
            "popularity": 0.49,
            "reads": 96864,
            "tags": ["design", "health", "culture"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 73,
            "likes": 315,
            "popularity": 0.13,
            "reads": 8966,
            "tags": ["design"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 13,
            "likes": 230,
            "popularity": 0.31,
            "reads": 64058,
            "tags": ["design", "tech"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 38,
            "likes": 105,
            "popularity": 0.45,
            "reads": 45896,
            "tags": ["design", "history"],
        },
    ]
}

# ENDPOINT: /api/posts?tags=design
API_RESPONSE_TAG = {
    "posts": [
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 4,
            "likes": 728,
            "popularity": 0.88,
            "reads": 19645,
            "tags": ["science", "design", "tech"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 11,
            "likes": 750,
            "popularity": 0.54,
            "reads": 6183,
            "tags": ["science", "design"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 13,
            "likes": 230,
            "popularity": 0.31,
            "reads": 64058,
            "tags": ["design", "tech"],
        },
        {
            "author": "Adalyn Blevins",
            "authorId": 11,
            "id": 16,
            "likes": 749,
            "popularity": 0.29,
            "reads": 71754,
            "tags": ["design", "history"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 28,
            "likes": 713,
            "popularity": 0.8,
            "reads": 89173,
            "tags": ["design"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 36,
            "likes": 709,
            "popularity": 0.08,
            "reads": 65277,
            "tags": ["health", "design"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 38,
            "likes": 105,
            "popularity": 0.45,
            "reads": 45896,
            "tags": ["design", "history"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 41,
            "likes": 715,
            "popularity": 0.69,
            "reads": 47876,
            "tags": ["design", "health"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 42,
            "likes": 452,
            "popularity": 0.08,
            "reads": 39721,
            "tags": ["design"],
        },
        {
            "author": "Jaden Bryant",
            "authorId": 3,
            "id": 51,
            "likes": 487,
            "popularity": 0.01,
            "reads": 98798,
            "tags": ["design", "startups", "tech"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 56,
            "likes": 319,
            "popularity": 0.49,
            "reads": 96864,
            "tags": ["design", "health", "culture"],
        },
        {
            "author": "Zackery Turner",
            "authorId": 12,
            "id": 63,
            "likes": 682,
            "popularity": 0.62,
            "reads": 54374,
            "tags": ["culture", "design"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 73,
            "likes": 315,
            "popularity": 0.13,
            "reads": 8966,
            "tags": ["design"],
        },
        {
            "author": "Kinley Crosby",
            "authorId": 10,
            "id": 75,
            "likes": 733,
            "popularity": 0.98,
            "reads": 94910,
            "tags": ["science", "design", "culture"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 81,
            "likes": 552,
            "popularity": 0.09,
            "reads": 22975,
            "tags": ["design", "history"],
        },
        {
            "author": "Lainey Ritter",
            "authorId": 1,
            "id": 97,
            "likes": 382,
            "popularity": 0.83,
            "reads": 47484,
            "tags": ["politics", "science", "design", "culture"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 98,
            "likes": 934,
            "popularity": 0.5,
            "reads": 17307,
            "tags": ["design"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 100,
            "likes": 573,
            "popularity": 0.43,
            "reads": 89894,
            "tags": ["science", "design", "history"],
        },
    ]
}

# ENDPOINT: /api/posts?tags=design&sortBy=popularity
API_RESPONSE_SORT_BY = {
    "posts": [
        {
            "author": "Jaden Bryant",
            "authorId": 3,
            "id": 51,
            "likes": 487,
            "popularity": 0.01,
            "reads": 98798,
            "tags": ["design", "startups", "tech"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 36,
            "likes": 709,
            "popularity": 0.08,
            "reads": 65277,
            "tags": ["health", "design"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 42,
            "likes": 452,
            "popularity": 0.08,
            "reads": 39721,
            "tags": ["design"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 81,
            "likes": 552,
            "popularity": 0.09,
            "reads": 22975,
            "tags": ["design", "history"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 73,
            "likes": 315,
            "popularity": 0.13,
            "reads": 8966,
            "tags": ["design"],
        },
        {
            "author": "Adalyn Blevins",
            "authorId": 11,
            "id": 16,
            "likes": 749,
            "popularity": 0.29,
            "reads": 71754,
            "tags": ["design", "history"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 13,
            "likes": 230,
            "popularity": 0.31,
            "reads": 64058,
            "tags": ["design", "tech"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 100,
            "likes": 573,
            "popularity": 0.43,
            "reads": 89894,
            "tags": ["science", "design", "history"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 38,
            "likes": 105,
            "popularity": 0.45,
            "reads": 45896,
            "tags": ["design", "history"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 56,
            "likes": 319,
            "popularity": 0.49,
            "reads": 96864,
            "tags": ["design", "health", "culture"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 98,
            "likes": 934,
            "popularity": 0.5,
            "reads": 17307,
            "tags": ["design"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 11,
            "likes": 750,
            "popularity": 0.54,
            "reads": 6183,
            "tags": ["science", "design"],
        },
        {
            "author": "Zackery Turner",
            "authorId": 12,
            "id": 63,
            "likes": 682,
            "popularity": 0.62,
            "reads": 54374,
            "tags": ["culture", "design"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 41,
            "likes": 715,
            "popularity": 0.69,
            "reads": 47876,
            "tags": ["design", "health"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 28,
            "likes": 713,
            "popularity": 0.8,
            "reads": 89173,
            "tags": ["design"],
        },
        {
            "author": "Lainey Ritter",
            "authorId": 1,
            "id": 97,
            "likes": 382,
            "popularity": 0.83,
            "reads": 47484,
            "tags": ["politics", "science", "design", "culture"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 4,
            "likes": 728,
            "popularity": 0.88,
            "reads": 19645,
            "tags": ["science", "design", "tech"],
        },
        {
            "author": "Kinley Crosby",
            "authorId": 10,
            "id": 75,
            "likes": 733,
            "popularity": 0.98,
            "reads": 94910,
            "tags": ["science", "design", "culture"],
        },
    ]
}

# ENDPOINT: /api/posts?tags=design&direction=desc
API_RESPONSE_DIRECTION = {
    "posts": [
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 100,
            "likes": 573,
            "popularity": 0.43,
            "reads": 89894,
            "tags": ["science", "design", "history"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 98,
            "likes": 934,
            "popularity": 0.5,
            "reads": 17307,
            "tags": ["design"],
        },
        {
            "author": "Lainey Ritter",
            "authorId": 1,
            "id": 97,
            "likes": 382,
            "popularity": 0.83,
            "reads": 47484,
            "tags": ["politics", "science", "design", "culture"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 81,
            "likes": 552,
            "popularity": 0.09,
            "reads": 22975,
            "tags": ["design", "history"],
        },
        {
            "author": "Kinley Crosby",
            "authorId": 10,
            "id": 75,
            "likes": 733,
            "popularity": 0.98,
            "reads": 94910,
            "tags": ["science", "design", "culture"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 73,
            "likes": 315,
            "popularity": 0.13,
            "reads": 8966,
            "tags": ["design"],
        },
        {
            "author": "Zackery Turner",
            "authorId": 12,
            "id": 63,
            "likes": 682,
            "popularity": 0.62,
            "reads": 54374,
            "tags": ["culture", "design"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 56,
            "likes": 319,
            "popularity": 0.49,
            "reads": 96864,
            "tags": ["design", "health", "culture"],
        },
        {
            "author": "Jaden Bryant",
            "authorId": 3,
            "id": 51,
            "likes": 487,
            "popularity": 0.01,
            "reads": 98798,
            "tags": ["design", "startups", "tech"],
        },
        {
            "author": "Bryson Bowers",
            "authorId": 6,
            "id": 42,
            "likes": 452,
            "popularity": 0.08,
            "reads": 39721,
            "tags": ["design"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 41,
            "likes": 715,
            "popularity": 0.69,
            "reads": 47876,
            "tags": ["design", "health"],
        },
        {
            "author": "Tia Roberson",
            "authorId": 2,
            "id": 38,
            "likes": 105,
            "popularity": 0.45,
            "reads": 45896,
            "tags": ["design", "history"],
        },
        {
            "author": "Rylee Paul",
            "authorId": 9,
            "id": 36,
            "likes": 709,
            "popularity": 0.08,
            "reads": 65277,
            "tags": ["health", "design"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 28,
            "likes": 713,
            "popularity": 0.8,
            "reads": 89173,
            "tags": ["design"],
        },
        {
            "author": "Adalyn Blevins",
            "authorId": 11,
            "id": 16,
            "likes": 749,
            "popularity": 0.29,
            "reads": 71754,
            "tags": ["design", "history"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 13,
            "likes": 230,
            "popularity": 0.31,
            "reads": 64058,
            "tags": ["design", "tech"],
        },
        {
            "author": "Ahmad Dunn",
            "authorId": 7,
            "id": 11,
            "likes": 750,
            "popularity": 0.54,
            "reads": 6183,
            "tags": ["science", "design"],
        },
        {
            "author": "Elisha Friedman",
            "authorId": 8,
            "id": 4,
            "likes": 728,
            "popularity": 0.88,
            "reads": 19645,
            "tags": ["science", "design", "tech"],
        },
    ]
}

# ENDPOINT /api/posts?tags=design
UNSORTED_API_RESPONSE = {
    "4": {
        "author": "Elisha Friedman",
        "authorId": 8,
        "id": 4,
        "likes": 728,
        "popularity": 0.88,
        "reads": 19645,
        "tags": ["science", "design", "tech"],
    },
    "11": {
        "author": "Ahmad Dunn",
        "authorId": 7,
        "id": 11,
        "likes": 750,
        "popularity": 0.54,
        "reads": 6183,
        "tags": ["science", "design"],
    },
    "13": {
        "author": "Elisha Friedman",
        "authorId": 8,
        "id": 13,
        "likes": 230,
        "popularity": 0.31,
        "reads": 64058,
        "tags": ["design", "tech"],
    },
    "16": {
        "author": "Adalyn Blevins",
        "authorId": 11,
        "id": 16,
        "likes": 749,
        "popularity": 0.29,
        "reads": 71754,
        "tags": ["design", "history"],
    },
    "28": {
        "author": "Ahmad Dunn",
        "authorId": 7,
        "id": 28,
        "likes": 713,
        "popularity": 0.8,
        "reads": 89173,
        "tags": ["design"],
    },
    "36": {
        "author": "Rylee Paul",
        "authorId": 9,
        "id": 36,
        "likes": 709,
        "popularity": 0.08,
        "reads": 65277,
        "tags": ["health", "design"],
    },
    "38": {
        "author": "Tia Roberson",
        "authorId": 2,
        "id": 38,
        "likes": 105,
        "popularity": 0.45,
        "reads": 45896,
        "tags": ["design", "history"],
    },
    "41": {
        "author": "Rylee Paul",
        "authorId": 9,
        "id": 41,
        "likes": 715,
        "popularity": 0.69,
        "reads": 47876,
        "tags": ["design", "health"],
    },
    "42": {
        "author": "Bryson Bowers",
        "authorId": 6,
        "id": 42,
        "likes": 452,
        "popularity": 0.08,
        "reads": 39721,
        "tags": ["design"],
    },
    "51": {
        "author": "Jaden Bryant",
        "authorId": 3,
        "id": 51,
        "likes": 487,
        "popularity": 0.01,
        "reads": 98798,
        "tags": ["design", "startups", "tech"],
    },
    "56": {
        "author": "Elisha Friedman",
        "authorId": 8,
        "id": 56,
        "likes": 319,
        "popularity": 0.49,
        "reads": 96864,
        "tags": ["design", "health", "culture"],
    },
    "63": {
        "author": "Zackery Turner",
        "authorId": 12,
        "id": 63,
        "likes": 682,
        "popularity": 0.62,
        "reads": 54374,
        "tags": ["culture", "design"],
    },
    "73": {
        "author": "Rylee Paul",
        "authorId": 9,
        "id": 73,
        "likes": 315,
        "popularity": 0.13,
        "reads": 8966,
        "tags": ["design"],
    },
    "75": {
        "author": "Kinley Crosby",
        "authorId": 10,
        "id": 75,
        "likes": 733,
        "popularity": 0.98,
        "reads": 94910,
        "tags": ["science", "design", "culture"],
    },
    "81": {
        "author": "Bryson Bowers",
        "authorId": 6,
        "id": 81,
        "likes": 552,
        "popularity": 0.09,
        "reads": 22975,
        "tags": ["design", "history"],
    },
    "97": {
        "author": "Lainey Ritter",
        "authorId": 1,
        "id": 97,
        "likes": 382,
        "popularity": 0.83,
        "reads": 47484,
        "tags": ["politics", "science", "design", "culture"],
    },
    "98": {
        "author": "Tia Roberson",
        "authorId": 2,
        "id": 98,
        "likes": 934,
        "popularity": 0.5,
        "reads": 17307,
        "tags": ["design"],
    },
    "100": {
        "author": "Ahmad Dunn",
        "authorId": 7,
        "id": 100,
        "likes": 573,
        "popularity": 0.43,
        "reads": 89894,
        "tags": ["science", "design", "history"],
    },
}
